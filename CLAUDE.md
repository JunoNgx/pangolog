...
- Google OAuth access tokens expire after 1 hour. GIS silent refresh (`requestAccessToken({ prompt: "" })`) can fail on mobile PWA (iOS standalone mode) because the WebView is isolated from Safari's cookie store, so GIS cannot verify the Google session. It can also fail due to clock skew causing a token to expire mid-sync. When a Drive API 401 occurs, `useSync` calls `getValidToken(true)` to force a GIS refresh regardless of local token validity (bypassing the `isTokenValid` check, which may be stale due to clock skew). If the forced refresh fails (returns `null`), the sync silently resets to idle and the user stays "connected" - no logout. Only if the forced refresh succeeds but Drive still returns 401 (genuine revocation) does `useSync` call `setAuthToken(null)` and show a "please reconnect" toast.

...
- Users can choose to start fresh or carry over current local data upon providing Google Drive authorisation.
...
- `amount`: integer (Stored as minor units, e.g., 100 for $1.00 or 100 for 100 VND).
    - Must be > 0
- `year`: integer, derived field from `createdAt`
- `month`: integer, derived field from `createdAt`
